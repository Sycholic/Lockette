...
public class LocketteBlockListener implements Listener {
    ...
    public void onMoveItemEvent(InventoryMoveItemEvent event) {
        // putting original check back in cuz this crap so wacked atm...
        Lockette.log.info("------------------------------------------------------");
        Lockette.log.info("      Event      = " + event);
        Lockette.log.info("getSource()      = " + event.getSource());
        Lockette.log.info("getSHolder()     = " + event.getSource().getHolder());
        Lockette.log.info("getSClass()      = " + event.getSource().getHolder().getClass());
        Lockette.log.info("getDestination() = " + event.getDestination());
        Lockette.log.info("getDHolder()     = " + event.getDestination().getHolder());
        Lockette.log.info("getDClass()      = " + event.getDestination().getHolder().getClass());
        if (event.getSource().getHolder() instanceof Chest) {
            if (Lockette.isProtected(((Chest) event.getSource().getHolder()).getBlock())) {
                Lockette.log.info("Chest isProtected :" + Lockette.isProtected(((Chest) event.getSource().getHolder()).getBlock()));
            }
            event.setCancelled(true);
        }
        /*-----------Commented out for now because logic for checking with hoppers is seriously f'd up---------------    

         Boolean protectedSource = false;
         Boolean protectedDest = false;
         Boolean protectedDblSource = false;
         Boolean protectedDblDest = false;


         if ((event.getSource().getHolder()) instanceof BlockState) {
         Lockette.log.info("BlockState: Source.getholder() = " + (event.getSource().getHolder()));
         protectedSource = (Lockette.isProtected(((BlockState) event.getSource().getHolder()).getBlock()));
         Lockette.log.info("Blockstate source is protected: " + protectedSource);
         }
         if ((event.getDestination().getHolder()) instanceof BlockState) {
         Lockette.log.info("Destination.getholder() = " + (event.getDestination().getHolder()));
         protectedDest = (Lockette.isProtected(((BlockState) event.getDestination().getHolder()).getBlock()));
         Lockette.log.info("Blockstate destination is protected: " + protectedSource);
         }
         if (protectedSource ^ protectedDest) {
         Lockette.log.info("Source or Destination of a Blockstate item is protected we cancel event and return at this point.");
         Lockette.log.info(" ");
         event.setCancelled(true);
         return;
         }
         // Booya thats like 90% of the decisions... start the DoubleChest hell here..

         if (event.getSource().getHolder() instanceof Chest & !(event.getSource().getHolder() instanceof Hopper)) {
         Lockette.log.info("testfour");
         if (protectedDblSource = Lockette.isProtected(((Chest) event.getSource().getHolder()).getBlock())) {
         Lockette.log.info("testfive");
         }
         }
         if (event.getDestination().getHolder() instanceof DoubleChest & !(event.getSource().getHolder() instanceof Hopper)) {
         Lockette.log.info("testsix");
         if (protectedDblDest = Lockette.isProtected(((Chest) event.getDestination().getHolder()).getBlock())) {
         Lockette.log.info("testseven");
         }
         }
         if ((protectedSource ^ protectedDblDest) ^ (protectedDest ^ protectedDblSource)) {
         //if (protectedSource ^ protectedDblDest) {
         Lockette.log.info("testeight");
         event.setCancelled(true);
         return;
         }
         //  whew! LAST minecarts last because they cant be protected and rarely going to be checked...
         ...
         ------------Commented out for now because logic for checking is seriously f'd up -------------*/
    }
